...
At the moment, <a href="/" id="link" class="no-underline">Countries Sundries</a> covers a total of 164 images of countries and different international organizations.
...
```bash
curl -X GET "http://countries-sundries-api.com/image/flag?id_flag=1" -H "x-countries_sundries-key: YOUR_API_KEY" -H "x-api-version: 1"
```
...
```bash
curl -X GET "http://countries-sundries-api.com/image/flag?name=argentina" -H "x-countries_sundries-key: YOUR_API_KEY" -H "x-api-version: 1"
```
...
Allows you to request the flag of a specific country by id.\
```bash
curl -X GET "http://countries-sundries-api.com/image/coat-of-arm?id=1" -H "x-countries_sundries-key: YOUR_API_KEY" -H "x-api-version: 1"
```
...
Allows you to request the flag of a specific country by name.\
```bash
curl -X GET "http://countries-sundries-api.com/image/coat-of-arm?name=argentina" -H "x-countries_sundries-key: YOUR_API_KEY" -H "x-api-version: 1"
```